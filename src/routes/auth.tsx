  ...
  async function handleEmailAuth(e: React.FormEvent) {
    ...
    try {
      if (mode === "signin") {
        ...
        // Auto-confirm may be off; try sign-in
      ...
      <div className="w-full max-w-md">
        ...
        <div className="glass rounded-3xl p-6 md:p-8 space-y-5">
          <div className="flex gap-1 p-1 bg-white/5 rounded-xl">
            <button onClick={() => setMode("signin")} className={`flex-1 py-2 text-sm font-medium rounded-lg transition ${mode === "signin" ? "bg-emerald text-emerald-foreground" : "text-muted-foreground"}`}>Sign in</button>
            <button onClick={() => setMode("signup")} className={`flex-1 py-2 text-sm font-medium rounded-lg transition ${mode === "signup" ? "bg-emerald text-emerald-foreground" : "text-muted-foreground"}`}>Sign up</button>
          </div>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            {mode !== "forgot" && (
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  {mode === "signin" && (
                    <button type="button" onClick={() => setMode("forgot")} className="text-xs text-emerald hover:underline">Forgot?</button>
                  )}
                </div>
                <div className="relative">
            ...
            <Button type="submit" disabled={loading} className="w-full h-11 bg-emerald hover:bg-emerald/90 text-emerald-foreground font-semibold">
              {loading ? <Loader2 className="size-4 animate-spin" /> : mode === "signin" ? "Sign in" : mode === "signup" ? "Create account" : "Send reset link"}
            </Button>
          </form>
          ...
          {mode !== "forgot" && (
            <>
              ...
              <Button type="button" variant="outline" onClick={handleGoogle} disabled={loading} className="w-full h-11 bg-white/5 border-white/10 hover:bg-white/10">
